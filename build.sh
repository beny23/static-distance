...
gunzip -c ukpostcodes.csv.gz | awk -F, '{printf "%s,%s,%.4f,%.4f\n", $1, $2, $3, $4}' | awk -F, -f filter_postcodes.awk > target/ukpostcodes.csv
...
awk 'BEGIN { FS="\t"; OFS=","; } $8 ~ /^(PPL|ADM)/ && $5 > 49 && $5 < 61 && $6 > -8 && $6 < 2 { gsub(/[^a-zA-Z0-9]/, "", $2); printf "%s,%s,%.4f,%.4f,%s\n", $8, $2, $5, $6, $15 }' GB.txt \
| sort -rnk5,5 -t, | cut -f1-4 -d, | tr '[:lower:]' '[:upper:]' | sort -u -k2,2 -t, | awk -F, -f split_locations.awk
